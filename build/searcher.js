...
class FileSearcher {
    ...
    async searchCode(options) {
        const results = [];
        const { query, fileTypes, maxResults = 20, caseSensitive = false, useRegex = false, includeContext = true, } = options;
    ...
    searchInFile(metadata, pattern, includeContext) {
        const results = [];
    ...
    getContextLines(lines, targetLine, contextSize) {
    ...
    calculateRelevanceScore(line, matches, metadata) {
        let score = 0;
    ...
    async findFiles(pattern, maxResults = 50) {
        const files = [];
    ...
    async findSymbols(symbolName, symbolType) {
        const results = [];
        ...
        const uniqueResults = results.filter((result, index, array) => array.findIndex(r => r.file === result.file && r.lineNumber === result.lineNumber) === index);
        uniqueResults.sort((a, b) => b.score - a.score);
        return uniqueResults.slice(0, 50);
    }
    async findSimilarFiles(filePath, similarity) {
    ...
    calculateStructuralSimilarity(file1, file2) {
    ...
    calculateContentSimilarity(file1, file2) {
    ...
    calculateImportSimilarity(file1, file2) {
