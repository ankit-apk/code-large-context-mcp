...
server.tool("search-code", "Search for code patterns, functions, classes, or text across the indexed repository.", {
    ...
    try {
        ...
        const formattedResults = results.map(result => `📁 ${result.file} (line ${result.lineNumber})\n` +
...
server.tool("get-repo-stats", "Get comprehensive statistics and overview of the indexed repository.", {}, async () => {
    try {
        if (!indexer) {
            return {
                content: [
...
function createDependencyTree(graph) {
    ...
    const result = [];
    function traverse(node, depth = 0, prefix = '') {
    ...
    const rootNodes = graph.nodes.filter(node => !graph.edges.some(edge => edge.to === node)).slice(0, 10);
